...
public class Day3_CrossedWires_UsingLines {
    public static final Coordinate startingCoordinate = new Coordinate(0, 0);
    ...
    public static int runA(String input) {
        ...
        List<StraightLine> linesFromRoute1 = getLinesFromRoute(split[0]);
        List<StraightLine> linesFromRoute2 = getLinesFromRoute(split[1]);

        // Go over all lines from route 1
        return linesFromRoute1.stream()
                .map(line -> getCoordinatesWhereLineCrossesWithRoute(linesFromRoute2, line))
                .flatMap(List::stream)
                .filter(coordinate -> !coordinate.equals(startingCoordinate))
                .mapToInt(coord -> coord.getManhattanDistance(startingCoordinate))
                .min()
                .getAsInt();

    }

    private static List<Coordinate> getCoordinatesWhereLineCrossesWithRoute(List<StraightLine> linesFromRoute2, StraightLine line) {
        // Check if any of the lines in route 2 crosses the line from route1
        return linesFromRoute2.stream()
                .map(line2 -> line2.crossesLineAt(line))
    ...
    public static List<StraightLine> getLinesFromRoute(String route) {
        List<StraightLine> linesList = new ArrayList<>();

        Arrays.stream(route.split(",")).forEach(routePart -> addRouteLineToCoords(routePart, linesList));
        return linesList;
    }

    public static void addRouteLineToCoords(String routePart, List<StraightLine> lineList) {
        // example routeparts: R1008 L339 U12 D965
        int steps = Integer.parseInt(routePart.substring(1));
        switch (routePart.substring(0, 1)) {
            case "R": createLineFromLastCoordinate(steps, lineList, true); break;
            case "L": createLineFromLastCoordinate(-steps, lineList, true); break;
            case "U": createLineFromLastCoordinate(steps, lineList, false); break;
            case "D": createLineFromLastCoordinate(-steps, lineList, false); break;
            default:
                throw new UnsupportedOperationException("This isn't a direction." + routePart.substring(0, 1));
        }
    }

    public static void createLineFromLastCoordinate(int steps, List<StraightLine> lineList, boolean horizontal) {
        Coordinate startCoordinate = lineList.size() == 0 ? startingCoordinate : lineList.get(lineList.size() -1).getEndCoordinate();
        Coordinate endCoordinate = startCoordinate.clone();
        if(horizontal) {
            endCoordinate.y += steps;
        } else {
            endCoordinate.x += steps;
        }
        lineList.add(new StraightLine(startCoordinate, endCoordinate));
    }

    public static int getCombinedDistanceWhereLineCrossesWithRoute(List<StraightLine> route, Line line) {
        throw new UnsupportedOperationException("Didn't build this (yet?) :)");
    }
